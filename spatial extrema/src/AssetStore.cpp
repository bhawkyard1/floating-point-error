...
{
		std::cout << "Loading mesh from " << g_resourceLocation + _path << '\n';
		ngl::Obj * mesh = new ngl::Obj(g_resourceLocation + _path);
		if(mesh == nullptr)
    {
...
void AssetStore::loadTexture(const std::string &_id, const std::string &_path)
{
    std::cout << "Loading texture from " << _path << '\n';
		SDL_Surface * surf = IMG_Load((g_resourceLocation + _path).c_str());
    GLuint tex;
    if(surf != nullptr)
